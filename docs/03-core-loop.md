...
*（步 7 的追加：这一步兑现了。`replayAgentState` 折叠事件时调用的就是这个 `reduce`，
它没有第二份推进逻辑；测试的比较对象是**同一次实时运行**里的状态，而每一条断言都过了。
顺带一提，回放里唯一一处需要「照抄实时语义」的地方，正好是这个函数的一处不那么显眼的
约定：`call_tool` 的意图不在决策那一刻进状态，而在观测到位时进——见
`docs/07-durability-replay.md` 边界决定 5。）*
