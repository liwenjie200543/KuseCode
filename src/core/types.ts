 ...
 * 一次工具调用让我们**看到了**哪一段材料。
 *
 * 它与 `Evidence` 长得像，但说的是两件相反的事，所以是两个类型：
 * `Evidence` 是**断言**（"我认为这里写着这个"），`MaterialRef` 是**事实**
 * （"这次调用确实取回了这一段"）。核对一条论断，就是问一句
 * 「它引用的这一段，有没有在事实里出现过」。
 *
 * `lines: null` 的含义是"这个路径存在，但这次没有看到它的任何一行"
 * （例如 `list_dir` 列出来的条目）：它足以支撑"这个文件存在"，
 * 支撑不了"第 7 行写着什么"。
 */
