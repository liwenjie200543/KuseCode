 ...
 * 事件日志 —— 一次 Run 的**唯一真相**。
 *
 * 这里只有契约和一份内存实现；真正活下来的那份（一行一事件的 JSONL）在
 * `src/store/run-log-jsonl.ts`（步 7 落地）。两个位置分开是刻意的：
 * **契约属于 Runtime，载体属于存储。** 换载体（文件、数据库、远端）不该改契约，
 * 也不该改驱动它的代码——步 7 落地时，这个文件一个字都没改。
 *
 * 步 2 的文档把这条接口推迟到步 7，前提是「等真正实现持久化时一并定」——
 * 那时还没有驱动方。步 4 有了驱动方，也就有了「事件落在哪里」这个必须现在回答的问题：
 * 一次 Run 的产物是一串有序事件，而「有序」必须有地方成立。所以接口在这里落地。
 *
 * 步 7 还把这份契约的**读法**变成了可推导的：回放（`src/runtime/replay.ts`）折叠
 * `read` 出来的事件，用的是步 3 的同一个 `reduce`。所以那条「只追加」的不变量
 * 不只是洁癖——`read` 的产物是回放的输入，允许洞就等于允许不可解释的输入。
 *
 * ## 为什么 `append` 是异步的
 *
 ...
 * 让 JSONL 实现（`src/store/run-log-jsonl.ts`）有一个可以逐条对照的行为定义——
 * 它比这一份严得多（序列化往返、残骸处理、路径校验），但「只追加、sequence 连续」
 * 这一条两边共用同一个 `assertAppendOnly`，不是各写一份。
 */
